...
export interface ProcessedData {
  ...
  metadata: {
    ...
    missingMetricsRows?: number;
    totalRows?: number;
  };
}

// 处理Excel文件
async function parseExcel(buffer: Buffer): Promise<ProcessedData> {
...
// 处理CSV文件
async function parseCsv(buffer: Buffer): Promise<ProcessedData> {
...
// 处理XML文件
async function parseXml(buffer: Buffer): Promise<ProcessedData> {
...
// 处理PDF文件
async function parsePdf(buffer: Buffer): Promise<ProcessedData> {
  ...
  for (let i = 1; i <= pdf.numPages; i++) {
    ...
    textContent.push(...content.items.map(item => (item as any).str));
  }
...
// 辅助函数：将XML数据扁平化
function flattenXmlData(obj: any, prefix = ''): any[] {
  let result: any[] = [];
...
const financialMetricsMapping = {
  ...
  ebit: ['ebit', 'operating_income', 'operating_profit', 'operatingincome', 'operatingprofit', 'EBIT', '营业利润'],
  netIncome: ['net_income', 'netincome', 'net_profit', 'netprofit', 'profit_after_tax', '净利润', '归属于母公司股东的净利润'],
  depreciation: ['depreciation', 'depreciation_amortization', 'dep_amort', '折旧与摊销', '折旧'],
  capex: [
    'capital_expenditure', 
    'capex', 
    'capital_spending', 
...
// 验证必需的财务指标
function validateRequiredMetrics(data: any): FinancialMetrics | null {
  ...
  Object.keys(data).forEach(column => {
    ...
    for (const [metric, possibleNames] of Object.entries(financialMetricsMapping)) {
      if (possibleNames.some(name => {
        ...
        if (!isNaN(value)) {
          metrics[metric as keyof FinancialMetrics] = Math.abs(value); // 使用绝对值处理负数
  ...
  // 检查是否所有必需的指标都找到了
  const missingMetrics = Object.keys(financialMetricsMapping).filter(
    metric => !foundColumns.has(metric)
  );
...
function calculateWorkingCapital(data: any): number {
  const currentAssets = findValue(data, ['current_assets', 'currentassets', '流动资产', '流动资产合计']);
  const currentLiabilities = findValue(data, ['current_liabilities', 'currentliabilities', '流动负债', '流动负债合计']);
...
// 辅助函数：查找值
function findValue(data: any, possibleNames: string[]): number | null {
  // 首先尝试直接匹配
...
// 主解析函数
export async function parseFile(buffer: Buffer, fileType: string): Promise<ProcessedData> {
...
export { FinancialMetrics, financialMetricsMapping, validateRequiredMetrics, calculateWorkingCapital, findValue };
